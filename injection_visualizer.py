...
def generate_with_injection(
    ...
    """    
    Performs DDIM sampling with deterministic noise and injects a guidance
    latent at a specified timestep.
    """
    device = cfg.compute.device
    model.inference_scheduler.set_timesteps(cfg.generation.num_inference_steps, device=device)

    # Encode text prompts for both unconditional (empty) and conditional guidance.
    uncond_embeddings = model._encode_text([""])
    text_embeddings = model._encode_text([prompt])
    text_embeddings = torch.cat([uncond_embeddings, text_embeddings])
    
    # Generate the initial random latent vector.
    ...
    for i, t in enumerate(tqdm(model.inference_scheduler.timesteps, desc=desc)):
        ...
        # At the specified injection step, replace the current latent with a
        # noised version of the guidance latent.
        if i == injection_step:
        ...
        # --- Standard DDIM Step with Classifier-Free Guidance ---
        ...
        with torch.no_grad(), torch.autocast(device_type=device, dtype=model.torch_dtype):
            ...
            # Combine predictions to guide the generation towards the text prompt.
        ...
        # --- Snapshot Capturing ---
        # Periodically decode the current latent to capture the generation process.
        if (i + 1) % 10 == 0:
            with torch.no_grad():
...
def save_individual_image(image_tensor: torch.Tensor, output_path: Path):
    ...
    # Normalize tensor from [-1, 1] to [0, 1] range for visualization.
...
def save_grid_plot(
    ...
    """Creates and saves an annotated grid plot summarizing the experiment."""
    if not snapshots:
    ...
    title = f"Conditional Injection Analysis for Class: '{class_name.upper()}'"
    ...
    save_path_annotated = output_dir / f"annotated_grid_{class_name}"
...
def run_visualization(cfg: DictConfig):
    ...
    # Load the guidance dataset
    log.info(f"Loading guidance data from: {cfg.paths.guidance_data}")
    try:
        if cfg.paths.coco_image_subdir is None:
            dataset = DiffusionDataset(data_path=cfg.paths.guidance_data)
        else:
            dataset = DiffusionDataset(data_path=cfg.paths.guidance_data, image_folder=cfg.paths.coco_image_subdir)
    ...
    for class_name in class_names:
        ...
        class_items = dataset.class_to_items_map.get(class_name, [])
        if not class_items:
            log.warning(f"No items found for class '{class_name}'. Skipping.")
            continue

        # Select a random subset of images from the class to serve as guidance.
        num_examples = min(cfg.generation.examples_per_class, len(class_items))
        if num_examples < cfg.generation.examples_per_class:
            log.warning(f"Found only {len(class_items)} items for '{class_name}'. Will process {num_examples} examples.")
        ...
        for j, chosen_item in enumerate(chosen_items):
            ...
            log.info(f"Base directory for this example: {example_output_dir}")
            ...
            guidance_image_tensor = sample["images"].unsqueeze(0).to(device)
            log.info(f"Selected guidance image for '{class_name}' with prompt: '{prompt}'")

            # Pre-encode the guidance image into the VAE's latent space.
            with torch.no_grad():
                guidance_latent = model._encode_vae(guidance_image_tensor)
            
            all_snapshots_for_grid = []
            # --- Iterate over the configured injection strengths (lambda values) ---
            for i, lambda_val in enumerate(cfg.generation.lambda_values):
                # Lambda is a fraction of the total inference steps, determining the injection point.
                ...
                # Create a subdirectory to store the progressive snapshots for the current lambda.
                lambda_steps_dir = example_output_dir / f"lambda_{lambda_val:.1f}"
                lambda_steps_dir.mkdir(parents=True, exist_ok=True)
                
                # Save each captured snapshot from the denoising process.
                for k, snapshot_tensor in enumerate(snapshots_for_lambda):
                    step_number = (k + 1) * 10
                    filename = f"step_{step_number:03d}.png"
                    snapshot_path = lambda_steps_dir / filename
                    save_individual_image(snapshot_tensor, snapshot_path)
                log.info(f"Saved {len(snapshots_for_lambda)} progressive snapshots to: {lambda_steps_dir}")
